...
function findRowsNeedingHubDBCreation(allData, sheet, changes) {
	...
	changedUniqueIds.forEach((uniqueId) => {
		...
		if (!data) return;
		const hasRequiredData = data.date_and_time && data.team;
		...
		for (const rowInfo of rowsNeedingCreation) {
			...
			const payload = {
				...
				data: {
					...
					event_no: rowInfo.data.event_no,
					lane_no: rowInfo.data.lane_no,
					boat: rowInfo.data.boat,
					category: rowInfo.data.category,
					round: rowInfo.data.round,
					team: rowInfo.data.team,
					position: rowInfo.data.position,
					time: rowInfo.data.time,
				},
				metadata: {
